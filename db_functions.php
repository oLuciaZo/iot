...
class DB_Functions {
    ...
    public function getAllDevices($con){
      ...
      $result = mysqli_query($con,"SELECT `int_name`,`data_mac`,`data_humidity`,`data_temp`,`data_time` FROM iot_data INNER JOIN iot_inventory ON `data_mac` = `int_mac` WHERE data_no IN (SELECT MAX(data_no) FROM iot_data GROUP BY data_mac)  AND `data_time` >= CURDATE()");
      return $result;
    }
